...
| 11 | OIDC account takeover: `preferred_username` (unverified) preferred over `email`, no `email_verified` | HIGH (auth bypass) *if IdP allows user-set preferred_username* | Design flaw CONFIRMED in-code; exploitability IdP-dependent | external, IdP-gated |
...
### LEAD [MED/LOW — mine, admin-gated] Control-plane SSRF via CAS backend Location
`CASBackendService.Create/Update` → `ValidateAndExtractCredentials(req.Location, ...)` (casbackend.go:87) synchronously, and background checker `PerformValidation` (casbackend.go:886) connects to `backend.Location` (user-supplied OCI/S3/Azure endpoint). Potential control-plane→arbitrary-host SSRF (IMDS/internal). BUT gated by `PolicyCASBackendCreate`/`Update` (authz.go:374) = admin roles, NOT in project-token DefaultAuthzPolicies. Lower priority. Open: does ValidateAndExtractCredentials actually make a network call for S3 custom-endpoint / OCI host, and can Location be an arbitrary http host?
